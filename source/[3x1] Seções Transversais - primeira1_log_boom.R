...
for(j in unique(df2$cenario_n)){
    ...
    for (k in c(11)){
        ...
        # listar os painéis que o log boom deve aparecer
        
        if(dfc$cenario[1]== 'Cenário 1\ncom log boom' |
           dfc$cenario[1]== 'Cenário E\ncom log boom' |
           dfc$cenario[1]== 'Cenário 1\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário E\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário B\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário F\ncom log boom e desviador/retentor'){
        plot1 <- plot1 +
                geom_path(data = lbl1, 
                          aes(x = x,y = y), 
        ...
        # listar os painéis que o desviador/retentor deve aparecer
        if(dfc$cenario[1]== 'Cenário 1\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário E\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário B\ncom log boom e desviador/retentor' |
           dfc$cenario[1]== 'Cenário F\ncom log boom e desviador/retentor'){
        ...
        plot <- (plot4+plot1)/plot2+
            plot_annotation(
                title = paste0(dfc$cenario[1]),
                subtitle = paste0("Seção ", dic_sec2$local_n[1]),
        ...
        ggsave(
            filename = sprintf("%s [S%s].png",gsub("/","",gsub("\n"," ",dfc$cenario[1])), dic_sec2$local_n[1]),
            path = nRTools::createDir(sprintf("img/[3x1] Seção Transversal/%s - primeira", gsub('/', ' - ', nome))),
            plot = plot,
            device = "png",
            width = 20,
            height = 18,
