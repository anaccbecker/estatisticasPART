...
plot1 <- 
    ...
    geom_point(data = df_secao %>% group_by(cenario, particula, t) %>% slice(1), 
               aes(x = x, y = y), 
...
plot2 <- df_secao %>% group_by(cenario, particula, t) %>% slice(1) %>% 
    ...
    scale_y_continuous(breaks = c(0,-20,-40), limits = c(-50,1), minor_breaks = c(seq(0,-50,by=-5)))+
...
tabela <- df_secao %>% group_by(cenario, particula, t) %>% slice(1) %>% 
...
plot4 <-  tabela%>% 
  ...
  scale_x_continuous(limits = c(-55,5))+
  ...
  #labs(title = 'Tempo: {frame_time}', x = 'GDP per capita', y = 'life expectancy') +
  transition_time(24*(t-.tmin)) +
  ease_aes('linear')
#plot4
anim_save("teste.gif", animation = plot4)
...
ggsave(
    filename = sprintf("Cenario %s [%s] - todas.png",dfc$cenario[1], df_secao$Secao[1]),
